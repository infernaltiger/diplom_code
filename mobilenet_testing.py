...
class mobilenet_mod(nn.Module):
    def __init__(self):
        ...
        self.model.classifier[3] = nn.Linear(1024, 1)
    def forward(self, x):
            return torch.sigmoid(self.model(x))


def train_model(model, train_loader, val_loader, epochs=EPOCHS, lr=LEARNING_RATE, patience=PATIENCE):
    # Аналогично обученгию ResNet, но с другим алгоритмом оптимизации
...
#save_history_to_csv(history, "mobilenet_res_v1_8min.csv")

load_model(model, "mobilenet_88.pth")
